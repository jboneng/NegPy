...
| Diamond / sheared scene (objects lean) | (1) Image X shrunk to the AHB shading width while USB still paced the full line — Full window @1800 must stay ~2592 px; pad the shading table up. (2) Odd crop USB width at 1800 (e.g. 2455) — output width must stay even (`optical_span_alignment` + even pixel count). Delete `plustek_calib` (cache v9+) and rescan — log should show even `pixels=` |
