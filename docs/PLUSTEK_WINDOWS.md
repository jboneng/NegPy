...
| Negative very dark / positive washed bright (vs SilverFast) | Host path stretched to *home* chrome white while the film window is dimmer, so film base sits far below sensor white and NegPy meters wrong. Log should show `host calib exposure makeup gain=…`. Rescan (no cache delete needed) |
